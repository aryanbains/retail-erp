  ...
  for (const category of categories) {
    ...
    for (let i = 0; i < categoryProducts.length; i++) {
      ...
      products.push({
        ...
        reorderLevel,
        status:
          stockQty === 0 ? 'Out of Stock' : stockQty < reorderLevel ? 'Low Stock' : 'Active',
      });
    }
  }
...
export function generateMockEmployees() {
  ...
  for (let i = 0; i < 15; i++) {
    employees.push({
      ...
      leaveBalance: Math.floor(Math.random() * 15) + 7,
      dateOfJoining: new Date(Date.now() - Math.random() * 3 * 365 * 24 * 60 * 60 * 1000),
    });
  }
